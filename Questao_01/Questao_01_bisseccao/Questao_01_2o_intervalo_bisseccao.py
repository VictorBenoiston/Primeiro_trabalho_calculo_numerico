...
x = np.arange(-7, 5.5, 0.5)
print(x)
plt.plot(x, np.cos(x) + np.exp(x))
plt.show()"""

# Utilizando o método da bisseccao para o intervalo [-2; -1.5]:


def test(x):
    valor_aplicado = (np.cos(x) + np.exp(x))
    return valor_aplicado


def bisseccao():
    c = 0
    valores_a = [-1.5]  # Valores positivos
    valores_b = [-2]  # Valores negativos
    x_barra_aplicados = []
    x_barra = []
    while True:
...
c_total = bisseccao()[1]
print('Questao 01, intervalo [-2; -1.5]:')
print(f'A raiz aproximada é: {raiz_aproximada_bissecao}')
print(f'Para encontrar essa raiz, foram necessárias {c_total} iterações.')  #iterações


"""# Criando a tabela com os valores obtidos
# Criando o arquivo
outWorkbook = xl.Workbook("Questao_01_2o_intervalo_bisseccao.xlsx")
