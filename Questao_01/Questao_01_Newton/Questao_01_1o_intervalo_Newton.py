...
# Utilizando o método de Newton para o intervalo [-5; -4.5]:
...
def derivada(x):
    resultado = -np.sin(x) + np.exp(x)
    return resultado

def newton():
    c = 0
    x_barra = [-4.75]  # O Xo é um chute inicial de qualquer valor no I. Nesse caso, o valor do meio
    x_aplicado = []
    while True:
...
# Criando o arquivo
outWorkbook = xl.Workbook("Questao_01_1o_intervalo_newton.xlsx")
